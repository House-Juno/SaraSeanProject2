...
app.selectQuestion = (charactersArr, selectedQuestion) => {
  const randomQuote = Math.floor(
    Math.random() * charactersArr[0].quotes.length
  );
  ...
  let quotesIndx = charactersArr[0].quotes.indexOf(
    charactersArr[0].quotes[randomQuote]
  );
...
app.renderQuestion = () => {
  ...
  setTimeout(() => {
    ...
    if (app.index < app.questionArray.length && app.startOver == false) {
      quizNumber.innerHTML = `${app.index + 1} <span> out of </span> ${
        app.questionArray.length
      }`;
      ...
      if (
        app.correctChoice === app.questionArray.length &&
        app.incorrectChoice === 0
      ) {
        quoteContainer.innerHTML = `
        ...
        app.correctChoice === 0 &&
        app.incorrectChoice === app.questionArray.length
      ) {
        quoteContainer.innerHTML = `
                <div class="quizScoreBoard">
                    <div id="quote" class="mainQuote">
